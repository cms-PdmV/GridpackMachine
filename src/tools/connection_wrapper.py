...
class ConnectionWrapper:
    ...
    def __init__(self, host, cert_file=None, key_file=None):
        ...
        self.logger.debug(
            "Host: %s, port %s, https: %s", self.host_url, self.port, self.https
        )
    ...
    def init_connection(self):
        ...
        """
        params = {"host": self.host_url, "port": self.port, "timeout": self.timeout}
    ...
    def api(self, method, url, data=None, headers=None):
        ...
        for attempt in range(1, self.connection_attempts + 1):
            ...
            try:
                self.connection.request(method, url, body=data, headers=all_headers)
                response = self.connection.getresponse()
                response_to_return = response.read()
                if response.status != 200:
                    self.logger.error(
                        "Error %d while doing %s to %s: %s",
                        response.status,
                        method,
                        url,
                        response_to_return,
                ...
                self.logger.error(
                    "Exception while doing a %s to %s: %s", method, url, str(ex)
                )
