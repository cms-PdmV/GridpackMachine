...
from src.utils.ssh_executor import SSHExecutor
...
class Controller:
    ...
    def update_repository_tree(self):
        ...
        run_command([f"cd {files_dir}", "git pull"])
        self.repository_tree = {
        ...
        with SSHExecutor(submission_host, ssh_credentials) as ssh:
            ...
            if gridpack_archive:
                ...
                if not Config.get("dev"):
                    ...
                    gridpack_directory = campaign_dict.get(
                        "gridpack_directory", gridpack_directory
                    )
                ...
                stdout, stderr, _ = ssh.execute_command(
                    f'rsync -e "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" {remote_directory}/{gridpack_archive} lxplus.cern.ch:{gridpack_directory}'
                )
                self.logger.debug(stdout)
                self.logger.debug(stderr)
    ...
    def create_mcm_request(self, gridpack):
        ...
        with SSHExecutor(tickets_host, ssh_credentials) as ssh:
            ...
            ssh.upload_file("mcm_gridpack.py", f"{remote_directory}/mcm_gridpack.py")
